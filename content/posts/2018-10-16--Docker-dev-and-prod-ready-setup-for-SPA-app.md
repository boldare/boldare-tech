...
PS. If you're familiar with Docker and don't need a step by step guide, check only [Multiple compose files](#multi-compose-files), [Configure it](#configure-it) and [Magic of multi-stage build](#magic). Code available on [GitHub](https://github.com/marcinlesek/docker-spa-setup-guide).
...
I know, you probably know what is Docker and how it's work, but if not, let me show you short quote from [What is a Container](https://www.docker.com/resources/what-container) which is great definition of Docker base block.

> A container is a standard unit of software that **packages up code and all its dependencies** so the application runs **quickly and reliably from one computing environment to another**. A Docker container image is a **lightweight, standalone, executable package** of software that includes everything needed to run an application: code, runtime, system tools, system libraries and settings.

I have bolded more important information which will help you understand how it works and how huge benefits it brings to our lives. Tightly package our application with all dependencies into light-weight block called **container** to fast run it on every environments.
...
Okey, we set up our own local machine, so we had all tools we need to start our journey. If you know a Docker a bit, you can have the first association with it as **Dockerfile**! Of course, you're right, but let me explain about it a bit more.. 
...
Dockerfile allows us to create a custom Docker image for our container from exisitng one and add our changes on build process. It's great, even we'll be using it here in next chapters, but **in production setup**, where *optimization, security and light-weight* of app is **crucial**. 

On the other hand, we have **docker-compose**, which could manage services (containers) by `docker-compose.yml` file. I can even try to say, that it helps us to simplify definition of our services by defining all containers in one file. For addition, it allows us to set ports, image to use, volumes, environments variables and many more! After two years with Docker in commercial products, I could say, for basic or mid-advanced projects, we could handle it with only [docker-compose](https://docs.docker.com/compose/). Benefits? **More time under reloading application** (mostly crucial for frontend developers wokring with watchers and hot reloads) because we'll be reducing our build time without any our custom commands for build own image.

Be more light and flexible, especially on development environment, when you need this a lot with continous changes you're making.
...
Now we can assume, that few sprints of development are gone and you have your dreamt *MVP ready to production*, but only development environment is working now - it's heavy, not optimized and with a lot of additional logs. Let's change it!
...
```
As you can see, it's changed a lot from `dev` version, so nothing could be passed up to our base `docker-compose.yml` file. Now we'll be using build process rather than ready-made image from Docker Hub and [nginx](http://nginx.org/) to serve our app. Of course you could also use [Apache](https://httpd.apache.org/) or another HTTP server. Our service is called `client`, just as in `dev` setup, because we're building this same app in another configuration, we won't to break our networks onnection (for example [using service name rather than IPs](https://docs.docker.com/docker-cloud/apps/service-links/#using-service-and-container-names-as-hostnames)). Let's take a closer look at it:

* `build:` - here we're specyfing our build process, how we would create our image for our production containers,
...
* `restart:` - container restart policy. We wan't our production to be down for long time due to some mistakes or not to start after server restart - `unless-stopped` will cover it for us, containers will be restarted always, except for the moment when we'll stop them manually (e.g. `docker-compose stop`). I don't recommend it on our dev machines, because it'll start always with our operating system and sometimes you also didn't code, so free up your resources.

This basic `docker-compose.prod.yml` **didn't handle everything, but it's really good start for production setup** (or good skieleton for it). For example, here we didn't handle HTTPS connection on 443 port (but it's easy to add, don't worry #trueStory).

Now let's build our `.env` file for production. But wait, we already have such a file `.env` - and **it's wrong**! For many reasons, as an example security, you shouldn't keep `.env` files in repository. It's realy easy to change them between deploys without changing any code and also it's very often to store in them some keys that couldn't be public. They should show us what it's necessary for application to work on a particular environment, but it should be completed manually (**especially for private keys!**). To be sure we didn't do it, we'll add record to `.gitginore`:
...
Now let's rename our `.env`s to `.env.dev.dist` where we'll storage our development variables setup and let's create `.env.prod.dist` for production. With this move, whenever you'll be changing environment or setting it for first time, you will have to do `cp .env.{ENV}.dist .env` correctly. Nothing special, but this must be kept in mind.
...
* `NODE_ENV` is set for `production`, it's necessary, because many JavaScript engines, frameworks and libraries checks this variable and optymize themselves regarding to its value,
* `NGINX_PORT` - new variable instead of `NODE_PORT`, used to specify on which port we want had our production app served from container by **nginx**.
...
Wow, what happend here? Let's look on **first stage**, where we'll **build** our app. 

1. Firsty, we pull `node:carbon-alpine` image (alpine is lightweight image without any not necessary tools) and name it `builder` in our Dockerfile space.
2. In second line, we're creating directory `/opt/app` where we'll have our root project directory.
3. Tell Docker, that this folder (`/opt/app`) in container is root, where we'll be executing everything later (with this we don't have to using abslute paths).
...
Probably you will think now - okey, we had our container, our app is built, why not to add second service as HTTP server in `docker-compose.prod.yml` file and serve this app? Here is place, **where mutli stage build enters**! Of course you're right, but in this container you'd the whole app source, dependencies (`node_modules` bigger than sun) and nothing to serve it (right now). Multi stage could make it thiner, with nginx and only with one service! Sounds cool? Let's check what is going on in second build stage.
...
7. After that, we need to set it up. Luckily, it could be only copying default configuration prepared earlier (it will be displayed below, due to fact configuring nginx could be place for another series of posts, we'll leave it for now).
8. Removing everything from default nginx serve directory (be sure we have clean space for our application).
9. **Magic is here!** Now we're copying only our build output from `builder` container (our `node` image where application was build) to our new container with nginx to default serve location. After this move, Docker **will automatically remove old, notused builder container** and we'll be have `nginx:alpine` container with **only built applicaiton files** as `client` service. **Amazing**!
10. Command which will be used to run container, here we're starting nginx with flag `daemon off` so he will be run in the background and serve our application.

Almost done!

### <a name="enter-stage"></a>Enter the stage!

Last thing to do, create space for nginx related properties/files (it's good way to storage files & configurations with their purpose, also it's easier to manage it in bigger projects):
...
This setup handle only HTTP and serve our `index.html`. You only need to change `server_name` property value to your own url address. After that, you only have to run it:
...
Starting my-app-spa_client_1 ... done
```
Now, you could check your site locally entering `localhost:{NGINX_PORT} and officialy say **hello my produciton app**! Whoooa!

## <a name="extras"></a>Extras

I hope it will help anyone to udnerstand how **multi-stage builds** works and how you could use it to improve your SPA app (and not only because for API it's also works perfectly, but it's topic for another post probably). You could find working code on my [GitHub](https://github.com/marcinlesek/docker-spa-setup-guide), so don't hestiate to check it if you'll force some troubles with this guide or ask me some questions here or on my [Twitter](https://twitter.com/marcinlesek), I will try to help you. :)
